...
class Kriging:
    def __init__(self, parameters, target, repetitions, db_name, transform=dont_transform, local_simulations=False):
        """ parameters
                are the parameters that should be estimated.
            target
               is the set of moments that should be attained by the calibration.
            db_name
                is a database into which the parameter -> result is saved.
            transform
                can be used before the calculation of the fitness_function if you need to transform
                data, for example calculate moments
            local_simulations
                if this is true the workers -- simulations are also run on the local machine.
    ...
    def initialize(self, initial_runs):
        """ does either load from the database or generates a the first samples
    ...
    def kriging(self, batch_size=28, kriging_batch_size=28, success=0.01, schlinge_start=2.0, schlinge_change=0.05):
        """ fits a gaussian_process, chooses the most promising candidates, centers a new search space arround them,
        uses a latin hypercube to create new ABM - simulations in this search space. If there we haven't found a
        better candidate as we had found in the previous round we make the search space smaller.
        Repeats until result is sufficiently close """
        INPUT, RESULT = 0, 1
        ...
        while True:
            ...
            print('find best')
            best_old = deepcopy(self.simulations.best())
            ...
            best_kriging_candidate = deepcopy(kriging_candidates.best())
            ...
            best_simulation_candidate = deepcopy(simulation_candidates.best())
            ...
            if (schlinge <= 0.001).all() and self.simulations.best()[RESULT] < success:
                print('simulation better than target result (%f<%f)' % (self.simulations.best()[RESULT], success))
                self.save_result()
                return self.simulations.best_dict(), self.test_not_flat(self.simulations.best_dict(), 20)
            key_pressed = get_key()
            if key_pressed == 'c' or key_pressed == 'i':
                ...
                logger.info("best                   %f" % self.simulations.best()[RESULT])
            ...
            print('before %f' % self.simulations.best()[RESULT])
            best_output, best_complete = self.test(self.simulations.best_dict(), 20)
            print('after %f' % self.simulations.best()[RESULT])
    ...
    def test_not_flat(self, input, repetitions, all=False):
        """ runs one input; returns simulation """
